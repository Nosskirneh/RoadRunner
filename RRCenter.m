...
@interface KPIPCMethod : NSObject
@property (nonatomic, readonly) id target;
@property (nonatomic, readonly) SEL selector;
- (instancetype)initWithTarget:(id)target selector:(SEL)selector;
@end

@implementation KPIPCMethod
...
- (void)addTarget:(id)target action:(SEL)action {
    ...
    KPIPCMethod *method = [[KPIPCMethod alloc] initWithTarget:target
                                                     selector:action];
...
- (void)removeAction:(SEL)action {
    NSString *messageName = [self _messageNameForSelector:action];
    if (!_methods[messageName])
        THROW(@"method not registered: %@", NSStringFromSelector(action));

    _methods[messageName] = nil;

    [_notificationCenter removeObserver:self name:messageName object:nil];
}
...
- (void)_messageReceived:(NSNotification *)notification {
    ...
    KPIPCMethod *method = _methods[messageName];
    if (!method)
...
- (void)_throwException:(NSString *)msg fromMethod:(SEL)method {
    ...
    NSException *myException = [NSException exceptionWithName:@"KPIPCException"
                                                       reason:reason
                                                     userInfo:nil];
    @throw myException;
}
